...
void draw() {
  ...
  case GAME_RUN: // In-Game
    ...
    for (int i = 0; i < cabbageX.length; i++) {
      ...
      if (isHit(cabbageX[i], cabbageY[i], SOIL_SIZE, SOIL_SIZE, 
...
boolean isHit(float ax, float ay, float aw, float ah, 
  float bx, float by, float bw, float bh) { // Requirement #3
...
String convertFramesToTimeString(int frames) {	// Requirement #4
  return nf(floor(frames/3600), 2) + ":" + nf((floor(frames/60) %60), 2);
}
